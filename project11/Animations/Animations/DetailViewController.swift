...
class DetailViewController: UIViewController {
    ...
    fileprivate let delay = 0.35
    fileprivate let scale = 1.4
    ...
    fileprivate func curvePath(_ endPoint: CGPoint, controlPoint1: CGPoint, controlPoint2: CGPoint) {
        let path = UIBezierPath() //곡선 직선 그리기
        path.move(to: self.animateView.center)
        path.addCurve(to: endPoint, controlPoint1: controlPoint1, controlPoint2: controlPoint2)
        
        // CAKeyFrameAnimation : 애니메이션을 keyFrame단위로 쪼개어 각각의 keyFrame마다 효과를 주거나 다른 애니메이션을 하도록 설정할수 있는 클래스
        let ani = CAKeyframeAnimation(keyPath: "position")
        ani.path = path.cgPath
        ani.duration = self.duration
        
        self.animateView.layer.add(ani, forKey: "animate position along path")
    ...
    @IBAction func didTapAnimate(_ sender: Any) {
        ...
        case "Multicolor":
            multiColor(UIColor.red, UIColor.blue)
